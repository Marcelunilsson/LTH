...
def student_move(board):
   ...
   """
   savedBoard = board
   avaMoves = list(env.available_moves())
   bm = avaMoves[0]
   for move in avaMoves:
      rewMax = -100
      rew = MiniMax(board, 3, False)
      if rew > rewMax: bm = move
      rewMax = max(rewMax, rew)
   env.reset(savedBoard)
   return bm


def MiniMax(board, depth, student):
   savedBoard = board
   if depth == 0:
      return 0
   if student:
      ...
      for move in env.available_moves():
         env.reset(savedBoard)
         sState, sReward, sDone, s_ = env.step(move) 
         if sDone: return sReward
         env.change_player()
         rew = MiniMax(board, depth -1, False)
         rewMax = max(rewMax, rew)
      return rewMax
   else:
      rewMin = 100
      for move in list(env.available_moves()):
         env.reset(savedBoard)
         env.change_player()
         sState, sReward, sDone, s_ = env.step(move)
         if sDone: return sReward
         env.change_player()
         rew = MiniMax(sState, depth-1, True)
         rewMin = min(rewMin, rew)
      return rewMin
