function cifar10_starter()
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(3,3,16,16)/sqrt(3*3*3/2), 'biases', zeros(16,1)),...
        'padding', [1 1]);
    ...
    net.layers{end+1} = struct('type', 'fully_connected',...
        'params', struct('weights', randn(1024,1024)/sqrt(1024/2), 'biases', zeros(1024,1)));
    net.layers{end+1} = struct('type', 'relu');
    ...
    save('models/cifar10_4F2Con1MP.mat', 'net');
